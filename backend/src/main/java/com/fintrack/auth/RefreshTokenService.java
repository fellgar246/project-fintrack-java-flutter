...
/**
 * Issues, rotates and revokes refresh tokens. The raw token only ever exists in memory and
 * in the response sent to the client; the database stores just its SHA-256 hash, so a leaked
...
public class RefreshTokenService {
    ...
    /**
     * Validates the raw token (exists, not revoked, not expired) and revokes it. Returns
     * the owning userId so the caller can issue a fresh pair. Throws
     * {@link BadCredentialsException} for any invalid/reused/expired token.
     */
    @Transactional
