...
public class DevSeedService {
  ...
  public int seedSampleData(UUID userId) {
    ...
    List<Category> expenseCategories =
        categoryRepository.findAllForUser(userId, CategoryKind.EXPENSE, false);
    List<Category> incomeCategories =
        categoryRepository.findAllForUser(userId, CategoryKind.INCOME, false);
  ...
  private void ensureBudgets(UUID userId, List<Category> expenseCategories) {
    ...
    for (Category category : expenseCategories) {
      ...
      if (budgetRepository
          .findByUserIdAndCategoryIdAndYearMonth(userId, category.getId(), yearMonth)
          .isPresent()) {
        index++;
        continue;
      }
