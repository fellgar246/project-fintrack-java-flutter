...
/**
 * Translates every exception a controller can throw into the RFC 7807 Problem Details shape from
 * §4.7. This does NOT cover 401/403 raised by the security filter chain itself (missing/invalid
 * token, access denied before a controller runs) — those go through {@link
 * RestAuthenticationEntryPoint} / {@link RestAccessDeniedHandler} instead, since they happen before
 * {@code @RestControllerAdvice} is in the picture. Both paths emit the same JSON shape.
 */
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ErrorResponse> handleValidation(
    ...
    List<ErrorResponse.FieldError> errors =
        ex.getBindingResult().getFieldErrors().stream()
            ...
            .toList();
    return build(
        HttpStatus.BAD_REQUEST, "Validation failed", "Request validation failed", request, errors);
  }
  ...
  public ResponseEntity<ErrorResponse> handleMalformedRequest(
    ...
    return build(
        HttpStatus.BAD_REQUEST, "Malformed request", "Request body is malformed", request, null);
  }
  ...
  public ResponseEntity<ErrorResponse> handleUnexpected(Exception ex, HttpServletRequest request) {
    ...
    return build(
        HttpStatus.INTERNAL_SERVER_ERROR, "Internal error", "Reference: " + traceId, request, null);
  }
