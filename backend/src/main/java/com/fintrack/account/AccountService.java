...
public class AccountService {
  ...
  public AccountResponse getById(UUID userId, UUID accountId) {
    AccountBalanceRow row =
        accountRepository
            .findOneWithBalance(userId, accountId)
  ...
  public AccountResponse create(UUID userId, AccountRequest request) {
    ...
    return accountRepository
        .findOneWithBalance(userId, saved.getId())
        .map(accountMapper::toResponse)
  ...
  /**
   * RB-02: archives when the account has transactions (as source or destination); physically
   * deletes otherwise.
   */
  @Transactional
