...
/**
 * Orchestrates registration, login, refresh and logout. The tricky parts live here on
 * purpose: the transactional boundary between creating a user and seeding their default
 * categories, normalizing the email before checking uniqueness, and turning a raced
 * UNIQUE-constraint violation into a 409 instead of letting it surface as a 500.
 */
@Service
public class AuthService {
    ...
    /**
     * Creates the user and seeds their default categories in the same transaction: if the
     * seed fails, the user insert is rolled back too. The email uniqueness check is
     * best-effort (existsByEmail) plus a hard backstop: the pre-check can't close a race
     * between two concurrent registrations, so a caught {@link DataIntegrityViolationException}
     * from the DB's UNIQUE constraint is what actually guarantees a 409 instead of a 500.
     */
    @Transactional
    ...
    private UserResponse toUserResponse(User user) {
        return new UserResponse(user.getId(), user.getEmail(), user.getName(), user.getBaseCurrency(), user.getCreatedAt());
    }
}
